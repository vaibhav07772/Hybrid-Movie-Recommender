...
Hybrid Movie Recommendation System combining Collaborative and Content-Based Filtering with Streamlit UI.
...
User–movie interaction data is collected from ratings
Similar movies are found using item-based collaborative filtering
Movie content similarity is calculated using TF-IDF on genres
Both scores are combined to generate a final ranking
Already watched movies are excluded
Top-N movies are recommended to the user
...
Enter User ID – choose a valid user from the dataset
Select a Movie – the movie to base recommendations on
Adjust α Slider – balance between collaborative filtering and content-based filtering
Click “Recommend Movies” – top-N movies will be displayed immediately
Example Output:
